...
func init() {
	orm.RegisterModel(new(GimnasioClases))
}

// AddGimnasioClases insert a new GimnasioClases into database and returns
// last inserted Id on success.
func AddGimnasioClases(m *GimnasioClases) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}
...
// GetAllGimnasioClases retrieves all GimnasioClases matches certain condition. Returns empty list if
// no records exist
...
// UpdateGimnasioClases updates GimnasioClases by Id and returns error if
// the record to be updated doesn't exist
func UpdateGimnasioClasesById(m *GimnasioClases) (err error) {
	o := orm.NewOrm()
	v := GimnasioClases{Id: m.Id}
...
// DeleteGimnasioClases deletes GimnasioClases by Id and returns error if
// the record to be deleted doesn't exist
func DeleteGimnasioClases(id int) (err error) {
	o := orm.NewOrm()
	v := GimnasioClases{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&GimnasioClases{Id: id}); err == nil {
