...
type Noticias struct {
	...
	CreadoEn          time.Time   `orm:"column(creado_en);type(timestamp with time zone);null"`
	ActualizadoEn     time.Time   `orm:"column(actualizado_en);type(timestamp with time zone);null"`
}
...
// GetNoticiasById retrieves Noticias by Id. Returns error if Id doesn't exist
